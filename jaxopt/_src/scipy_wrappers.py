...
class ScipyLeastSquares(ScipyWrapper):
  ...
  Attributes:
    ...
    ftol: the `ftol` argument for `scipy.optimize.least_squares`.
    xtol: the `xtol` argument for `scipy.optimize.least_squares`.
    gtol: the `gtol` argument for `scipy.optimize.least_squares`.
    x_scale: the `x_scale` argument for `scipy.optimize.least_squares`. If
      `array_like`, must be a PyTree with structure identical to the model
      parameters `x`.
    ...
    f_scale: the `f_scale` argument for `scipy.optimize.least_squares`.
    tr_solver: the `tr_solver` argument for `scipy.optimize.least_squares`.
    tr_options: the `tr_options` argument for `scipy.optimize.least_squares`.
    max_nfev: the `max_nfev` argument for `scipy.optimize.least_squares`.
    dtype: if not None, cast all NumPy arrays to this dtype. Note that some
      methods relying on FORTRAN code, such as the `L-BFGS-B` solver for
      `scipy.optimize.minimize`, require casting to float64.
    jit: whether to JIT-compile JAX-based values and grad evals.
  ...
  tr_options: Optional[Dict[str, Any]] = None
  ...
  def _cost_fun(self, params, *args, **kwargs):
    ...
    losses = self._rho(residuals / self.f_scale)
    return 0.5 * jnp.square(self.f_scale) * jnp.mean(losses)
  ...
  def __post_init__(self):
    ...
    if self.x_scale != 'jac' and not isinstance(self.x_scale, float):
      self.x_scale = jnp_to_onp(self.x_scale, self.dtype)
    # Avoids mutable defaults for dataclass field.
    self.tr_options = {} if self.tr_options is None else self.tr_options

    # Pre-compile useful functions.
    self._rho = LS_RHO_FUNS.get(self.loss, lambda z: z)
